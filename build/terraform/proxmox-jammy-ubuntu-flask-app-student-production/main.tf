...
resource "proxmox_vm_qemu" "load-balancer" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "sed -i 's/flask-app.service/nginx.service/' /etc/alloy/config.alloy",
...
resource "proxmox_vm_qemu" "frontend-webserver" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "sudo systemctl enable node-exporter.service",
      "sudo systemctl start node-exporter.service",
      "sed -i 's/REPLACE/${var.frontend-yourinitials}-vm${count.index}/' /etc/alloy/config.alloy",
      "sudo systemctl restart alloy.service",
      "sudo growpart /dev/vda 3",
      "sudo lvextend -l +100%FREE /dev/ubuntu-vg/ubuntu-lv",
      "sudo resize2fs /dev/ubuntu-vg/ubuntu-lv",
...
resource "proxmox_vm_qemu" "backend-database" {
  ...
  #target_node = data.vault_generic_secret.target_node.data[random_shuffle.nodename.result[0]]
  target_node = random_shuffle.nodename.result[0]
  clone       = var.logserver-template_to_clone
  os_type     = "cloud-init"
  memory      = var.logserver-memory
  cpu {
    cores   = var.logserver-cores
    sockets = var.logserver-sockets
  }
  skip_ipv6 = true
  scsihw    = "virtio-scsi-pci"
  boot      = "order=virtio0"
  ...
  provisioner "remote-exec" {
    ...
    # instance and configure the system to register the FQDN with the Consul DNS system
    inline = [
      "sudo hostnamectl set-hostname ${var.logserver-yourinitials}-vm${count.index}",
      "sudo sed -i 's/changeme/${random_id.id.dec}${count.index}/' /etc/consul.d/system.hcl",
      "sudo sed -i 's/replace-name/${var.logserver-yourinitials}-vm${count.index}/' /etc/consul.d/system.hcl",
      "sudo sed -i 's/ubuntu-server/${var.logserver-yourinitials}-vm${count.index}/' /etc/hosts",
      "sudo sed -i 's/FQDN/${var.logserver-yourinitials}-vm${count.index}.service.consul/' /etc/update-motd.d/999-consul-dns-message",
      "sudo sed -i 's/#datacenter = \"my-dc-1\"/datacenter = \"rice-dc-1\"/' /etc/consul.d/consul.hcl",
      "echo 'retry_join = [\"${var.consulip-240-prod-system28}\",\"${var.consulip-240-student-system41}\",\"${var.consulip-242-room}\"]' | sudo tee -a /etc/consul.d/consul.hcl",
      ...
      "sudo rm /opt/consul/node-id",
      "sudo systemctl restart consul.service",
      "sudo sed -i 's/0.0.0.0/${var.logserver-yourinitials}-vm${count.index}.service.consul/' /etc/systemd/system/node-exporter.service",
      "sudo systemctl daemon-reload",
      "sudo systemctl enable node-exporter.service",
      "sudo systemctl start node-exporter.service",
      "sed -i 's/REPLACE/${var.logserver-yourinitials}-vm${count.index}/' /etc/alloy/config.alloy",
      "sudo systemctl restart alloy.service",
      "sudo growpart /dev/vda 3",
      "sudo lvextend -l +100%FREE /dev/ubuntu-vg/ubuntu-lv",
      "sudo resize2fs /dev/ubuntu-vg/ubuntu-lv",
...
resource "proxmox_vm_qemu" "log-server" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "sed -i 's/REPLACE/${var.backend-yourinitials}-vm${count.index}/' /etc/alloy/config.alloy",
      "sed -i 's/flask-app.service/mariadb.service/' /etc/alloy/config.alloy",
      "sudo systemctl restart alloy.service",      
      "sudo growpart /dev/vda 3",
      "sudo lvextend -l +100%FREE /dev/ubuntu-vg/ubuntu-lv",
      "sudo resize2fs /dev/ubuntu-vg/ubuntu-lv",
