...
# Run this by typing: terraform apply
###############################################################################################
...
##############################################################################
resource "proxmox_vm_qemu" "flask-app" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "sudo sed -i 's/0.0.0.0/${var.yourinitials}-vm${count.index}.service.consul/' /etc/systemd/system/flask-app.service",
      "sudo systemctl daemon-reload",
...
output "proxmox_ip_address_default" {
  ...
  value = proxmox_vm_qemu.flask-app.*.default_ipv4_address
}
